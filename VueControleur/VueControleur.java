...
public class VueControleur extends JPanel implements Observer {
    ...
    private static class HexCellLabel extends JLabel { //représentation case hexagonale

        private final Polygon shape; //definition forme hexagonale
        private boolean lowered = false; //état de la case (affichage)

        public HexCellLabel(int width, int height) { //constructeur 
            super("", SwingConstants.CENTER); 
            shape = creerHexagone(width, height); //création forme hexagonale
        ...
        @Override
        public boolean contains(int x, int y) {//permet de détecter les clics uniquement à l'intérieur de l'hexagone
    ...
    private ImageIcon chargerIcone(String chemin, int taille) {
        ...
        ImageIcon icon = new ImageIcon(url);
        Image scaled = icon.getImage().getScaledInstance(taille, taille, Image.SCALE_SMOOTH);
        return new ImageIcon(scaled);
    }
    ...
    private void setEtatSmiley(EtatSmiley etat) {
        ...
        switch (etat) {
            case CLIC:
                smileButton.setIcon(smileyClickIcon);
                break;
            case PERDU:
                smileButton.setIcon(smileyLoseIcon);
                break;
            case GAGNE:
                smileButton.setIcon(smileyWinIcon);
                break;
            case NORMAL:
            default:
    ...
    private void ajouterEcouteSourisCase(JLabel jlab, int x, int y) {
        jlab.addMouseListener(new MouseAdapter() {
            ...
            public void mouseReleased(MouseEvent e) {
                ...
                if (jeu.isEnCours()) {
                    setEtatSmiley(EtatSmiley.NORMAL);
                }
            }
        });
    }
